...
class MetadataExtractor {
    ...
    private fun findDocumentNumbers(text: String): List<String> {
        ...
        val patterns = listOf(
            Pattern.compile("(?:Invoice|Policy|ID|Ref|Receipt|Bill)\\s?[#: ]+([A-Z0-9-]{4,20})", Pattern.CASE_INSENSITIVE),
            Pattern.compile("\\b[A-Z]{2}\\d{2}[A-Z]{1,2}\\d{4}\\b") // Generic ID format like Passport/License
        )
        ...
        patterns.forEach { p ->
            ...
            while (matcher.find()) {
                val match = if (matcher.groupCount() >= 1) matcher.group(1) else matcher.group()
